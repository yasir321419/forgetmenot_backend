...
const otherMileStoneCategory = async () => {
  try {
    // Categories to create (hardcoded list)
    const categoriesToCreate = [
      { name: 'Send facebook message' },
      { name: 'Buy/send a card' },
      { name: 'Make a donation' },
      { name: 'Call or check in' },
      { name: 'Light a candle or symbolic action' }
    ];
    ...
    for (const categoryData of categoriesToCreate) {
      const existingCategory = await prisma.otherOptionRemiderCategory.findFirst({
        where: {
      ...
      if (!existingCategory) {
        await prisma.otherOptionRemiderCategory.create({
          data: {
...
module.exports = otherMileStoneCategory;
